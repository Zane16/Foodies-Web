...
import { Users, Search, Mail, Calendar, CheckCircle2, Building2, UserCheck, UserX, Shield } from "lucide-react"
...
interface SuperAdminUser {
  ...
  full_name: string
  email: string
  role: 'customer' | 'vendor' | 'deliverer' | 'admin'
...
export default function SuperAdminUsersPage() {
  ...
  useEffect(() => {
    // Filter users based on search query, role filter, and organization filter
    let filtered = users
    ...
    // Organization filter
    if (orgFilter !== 'all') {
      filtered = filtered.filter(user => user.organization === orgFilter)
    }
  ...
  async function fetchUsers() {
    ...
    try {
      ...
      if (response.ok) {
        ...
        const orgs = Array.from(new Set(usersData.map((u: SuperAdminUser) => u.organization).filter(Boolean))) as string[]
  ...
  const handleDeactivate = (user: SuperAdminUser) => {
  ...
  const handleReactivate = async (user: SuperAdminUser) => {
  ...
  const otherRolesCount = users.filter(u => u.role !== 'admin').length
          ...
          <div className="grid grid-cols-1 md:grid-cols-5 gap-6 mb-8">
            ...
            <Card className="bg-white shadow-md border-0 hover:shadow-lg transition-shadow">
              <CardContent className="pt-6">
                <div className="flex items-center justify-between">
                  <div>
                    <p className="text-sm font-semibold uppercase tracking-wide" style={{ color: '#6B7280' }}>Admins</p>
                    <div className="text-3xl font-bold mt-2" style={{ color: '#10B981' }}>{adminsCount}</div>
                  </div>
                  <div className="w-12 h-12 rounded-full flex items-center justify-center" style={{ backgroundColor: '#D1FAE5' }}>
            ...
            <Card className="bg-white shadow-md border-0 hover:shadow-lg transition-shadow">
              <CardContent className="pt-6">
                <div className="flex items-center justify-between">
                  <div>
                    <p className="text-sm font-semibold uppercase tracking-wide" style={{ color: '#6B7280' }}>Other Roles</p>
                    <div className="text-3xl font-bold mt-2" style={{ color: '#1A202C' }}>{otherRolesCount}</div>
                  </div>
              ...
              <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-sm text-white/80 mt-1">Manage all users across all organizations</p>
                  </div>
                </div>
                        ...
                        >
                          <td className="px-6 py-4">
                            <div className="flex items-center gap-3">
                              <div
                                ...
                                style={{
                                  backgroundColor: user.role === 'admin' ? '#10B981' : '#5B5FDE'
                                }}
                              >
                          ...
                          <td className="px-6 py-4">
                            <div className="flex items-center gap-2">
                              ...
                              <span className="text-sm" style={{ color: '#6B7280' }}>{user.organization || 'N/A'}</span>
                            </div>
                          </td>
                          <td className="px-6 py-4">
